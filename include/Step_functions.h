...
bool load_slot(float volume_ml); // lock, pump water+air through the CURRENT slot, unlock - see Step_functions.cpp
void load_slot_full(int slot_manifold); // FULL production sequence (step_sampling + step_DNA_shield) at the CURRENT slot, then unlock - see Step_functions.cpp
