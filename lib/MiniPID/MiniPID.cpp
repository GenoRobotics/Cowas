...
double MiniPID::getOutput(double actual, double setpoint){
	...
	if(minOutput!=maxOutput && !bounded(output, minOutput,maxOutput) ){
		...
		// ! adapted from initial library
		// errorSum=error; 
		// Serial.println("MaxOutput : Error sum resetted to error");
	...
	// ! added && condition to inital library
	else if(maxIOutput!=0 && !bounded(output, -maxIOutput,maxIOutput)){
